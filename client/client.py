    ...
    for message in messages:
        ...
        for part in message.parts:
            if isinstance(part, (ToolCallPart, BuiltinToolCallPart)):
                ...
                tool_call_detected = True
                block = payload_block(part.content)
                label = f"📥 Tool response • `{part.tool_name}`"
                activity.append(f"{label}\n\n{block}" if block else label)
...
def init() -> None:
    ...
    if not st.session_state.get("_page_configured"):
        st.set_page_config(
            page_title="MyBooks MCP OAuth Client",
            layout="wide",
            page_icon=Path("../mybooks/static/mybooks/img/favicon.png"),
        )
        st.session_state._page_configured = True
    
    st.title("MyBooks MCP OAuth Client")
        ...
        with st.spinner("Initializing MCP Server connection…"):
            ...
            try:
                ...
                if response is not None and response.status_code == 401:
                    ...
                    st.session_state.tools = [{"name": tool.name, "description": tool.description} for tool in tools]
                    ensure_oauth_metadata(None)
...
def render_sidebar() -> None:
    ...
    with st.sidebar:
        ...
        st.divider()
        with st.expander("MCP Server", expanded=False):
            with st.spinner("Loading MCP tools list…"):
                ...
                st.write({"url": mcp_server.url})

                if tools_error:
                    st.error(tools_error)
                elif tools:
        ...
        if client.client_id:
            st.divider()
            with st.expander("Client Application", expanded=False):
